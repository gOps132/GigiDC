...
| Preset | Capabilities |
| --- | --- |
| `gigi-admin` | `capability.manage`, `plugin.install`, `job.admin`, `llm.provider.write`, `llm.provider.test`, `llm.provider.select`, `memory.read.guild`, `memory.manage.guild` |
...
Required Gigi capability: none for `context:none`; channel memory tools still require `memory.read.guild` when `context:channel` and tool routing are enabled.
...
`context:none` routes through the agent chat path without memory tools. `context:channel` lets the agent load permitted recent full-mode messages into a bounded context pack and allows the planner to choose current-channel memory tools such as `memory.count`, `memory.search`, and `memory.recent` when `/llm routing` allows it and the requester has memory permission.
...
Privacy and audit: the agent path records scoped metadata such as tool names, status, intent, source IDs, citation labels, restore handles, context-pack status markers, run status, and termination reason. It does not store raw prompt text, raw memory snippets, provider responses, or secrets in audit metadata or durable step observations. Follow-up snapshots omit memory evidence summaries and provenance from `memory.search` and `memory.recent` tool data.
...
Agent command privacy and audit: run management responses are ephemeral. Audit metadata stores action names, run IDs, confirmation IDs, tool names, and step indexes, but not raw prompts or raw tool arguments.
