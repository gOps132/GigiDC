...
`context:none` routes through the agent chat path without memory tools. `context:channel` allows the agent planner to choose current-channel memory tools such as `memory.count`, `memory.search`, and `memory.recent` when `/llm routing` allows it and the requester has memory permission.
...
Privacy and audit: the agent path records scoped metadata such as tool names, status, and intent. It does not store raw prompt text, raw memory snippets, provider responses, or secrets in audit metadata.
