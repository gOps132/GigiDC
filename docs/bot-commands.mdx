...
| Preset | Capabilities |
| --- | --- |
| `gigi-admin` | `capability.manage`, `plugin.install`, `job.admin`, `agent.analytics`, `llm.provider.write`, `llm.provider.test`, `llm.provider.select`, `memory.read.guild`, `memory.manage.guild` |
...
Required Gigi capability: `agent.analytics`.
...
Use this to inspect aggregate agent-runtime health for the current server. The response is ephemeral. If no period is provided, Gigi uses `7d`. If no runs match the selected period, Gigi returns a zero-run aggregate summary for that window.

Failure cases:

- requester lacks `agent.analytics`
