...
`context:none` routes through agent chat without memory fetches. `context:channel` lets the agent planner receive current-channel context. When `/llm routing` allows tools and the requester has `memory.read.guild`, Gigi can fetch retained messages from this channel, pack bounded snippets, run memory tools such as `memory.count`, `memory.search`, and `memory.recent`, then answer from those results.
...
Privacy and audit: dynamic context fetches are scoped to the current guild channel and require enabled memory plus `memory.read.guild`. Agent traces record metadata such as `run_id`, `step_index`, context scope, source type, result count, truncation flag, tool names, status, capabilities, and safe source/message IDs. Audit metadata does not store raw prompt text, raw fetched snippets, provider responses, embeddings, planned commands, or secrets.
...
Returns matching normalized snippets for the current channel with safe author metadata. Cross-channel search waits for channel-visibility filtering.
...
External app matching is live for guild mentions. Deterministic prefix matching runs first. If no prefix trigger matches and `/llm routing` is `dry-run` or `enabled`, semantic routing can call the configured `routing` model, validate the proposal against enabled manifests, and apply capability checks. `dry-run` returns the planned action only. `enabled` may dispatch public `send_message` actions; restricted actions still dry-run. It does not call LangChain, run harnesses, or use LLM output as dispatch authority.
