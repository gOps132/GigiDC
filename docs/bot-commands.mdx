...
Dynamic plugin capabilities such as `plugin.run.<id>` are future work and should appear only after plugin install/enable flow exists.

## Plugin Commands

All `/plugins` commands are live admin commands for the plugin catalog. They do not execute plugin behavior yet.
...
Lists approved plugin manifests in the catalog.
...
Fetches `gigi-plugin.json` from HTTPS, validates it, and stores it as an approved manifest.
...
Imports an uploaded `gigi-plugin.json` attachment, validates it, and stores it as an approved manifest.
...
Enables an approved plugin version for the current Discord server. This does not execute plugin commands yet.
...
Disables a plugin for the current Discord server.
...
Lists plugins enabled for the current Discord server.
