...
Dynamic external app capabilities such as `plugin.run.<id>` are future work and should appear only after plugin install/enable flow exists.
...
External app matching is live for guild mentions. It does not use an LLM, does not call LangChain, and does not run harnesses.
...
- no enabled external app manifest matches the text, so Gigi falls back to normal mention behavior
