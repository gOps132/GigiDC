...
Required Gigi capability: the matched manifest's `permissions` entry. If a manifest has no permissions, Gigi falls back to `plugin.run.<plugin-id>`.
...
- requester lacks the matched manifest permission and has no Discord administrator override
