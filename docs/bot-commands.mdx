...
## External App Dry-Run Messages

External app dry-run is live for guild mentions. It does not use an LLM, does not call LangChain, does not run harnesses, and does not send commands to the external app.
...
Audit: matched, denied, and failed dry-run attempts are audited without storing raw message text or the planned command.
...
All `/plugins` commands are live admin commands for the external app manifest catalog. They do not dispatch external app commands.
...
Enables an approved external app version for the current Discord server. This permits dry-run matching only; it does not dispatch external app commands.
...
Tests enabled external app trigger matching without using message content events and without sending commands to the external app.
...
- confirmed external app command dispatch
