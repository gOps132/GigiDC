...
LLM provider management is planned, not live. The provider registry should support OpenAI, Anthropic, Gemini, and future custom providers. V0 should expose guild-scoped provider credentials only, even though the internal credential model should reserve owner types for `guild`, `user`, and `tenant`. V1 can add personal BYOK commands for user-owned keys after policy, disclosure, and billing-owner rules exist.
...
- `llm.provider.write` for provider add, rotate, delete, and policy changes
- `llm.provider.test` for provider tests
- `llm.provider.select` for model profile selection

V0 guild/admin command direction:
