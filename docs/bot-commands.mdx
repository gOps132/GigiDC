...
These commands are not implemented yet. Guild memory should start off, require explicit per-channel opt-in, and run ingestion asynchronously so Discord message handling stays responsive.
...
```text
@sam mentioned "postgres" 14 times in indexed channels since Jan 1, 2026.
Top channels: #backend 11, #infra 3.
```

Count answers should use deterministic SQL over indexed messages. Gigi should not ask an LLM to guess aggregate counts.
...
Use this for exact or normalized aggregate questions. The default response should be ephemeral. `visibility:channel` may post a safe summary only.
...
Returns matching snippets with safe source metadata such as channel, author, timestamp, and message link when allowed by channel visibility.
...
Shows enabled channels, last sync, backlog, retention, and configured embedding model. Response should be ephemeral.
...
`metadata` mode can support exact counts without retaining raw searchable content. `full` mode permits search segments and embeddings after redaction. Retention is required before ingest.
