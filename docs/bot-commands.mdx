...
`context:none` routes through the agent chat path without memory tools. `context:channel` allows the agent planner to choose current-channel memory tools such as `memory.count` and `memory.search` when `/llm routing` allows it and the requester has memory permission.
