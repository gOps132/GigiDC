...
- Update architecture docs when runtime boundaries, storage, permissions, memory, or plugin behavior changes.
...
Future plugin work can include contributed `gigi-plugin.json` manifests for known Discord bots, private guild tools, or external plugin-style services.
...
- Include attribution for Discord docs, plugin docs, provider docs, and any examples used to build the manifest.
...
Gigi can validate and store approved manifests, but plugin execution is not live yet. Contributor manifests are preparation for later plugin catalog and guild enablement work.

Do not document a plugin as usable until the runtime can import, enable, authorize, audit, and execute that plugin behavior.
