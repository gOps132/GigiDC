...
Discord login is disabled by default through `GIGI_DISCORD_ENABLED=false`. When enabled, the gateway can publish `/ping`, `/permissions`, `/llm`, `/plugins`, `/ask`, and `/agent`, answer `/ping`, route DMs, route guild mentions, ignore ordinary unmentioned guild messages, and ignore bot-authored messages. External app manifest validation, storage, import, enable, disable, deterministic dry-run matching, semantic dry-run routing, guild mention chat fallback, aggregate agent stats, opt-in reply latency footers, and consented public `send_message` prefix dispatch have started. Provider HTTP text calls are live when a guild credential, model profile, and `GIGI_LLM_SECRET_KEY_BASE64` are configured. Restricted dispatch, retrieval, memory, and rich DM chat remain unavailable.
...
- `internal/jobs`: durable job queue contracts, SQL queue operations, and worker-pool foundation.
...
- `internal/agent`: agent runtime core for bounded runs, request planning, policy checks, tool execution, answer composition, confirmation, audit-friendly traces, durable run/step state, and current-channel context loading.
- `internal/contextbroker`: scoped context retrieval and packing layer for current-channel retained memory and bounded context packs. The pack builder assigns stable source IDs, citation labels, stale/invalidation markers, pinned-snippet priority, omitted-unchanged markers, and restore handles; permitted cross-channel guild memory, enabled plugin catalog summaries, and richer token budgeting remain future work.
...
The cognitive layer is evolving into the agent runtime core. Exact deterministic handlers remain first. If `/llm routing` is `dry-run` or `enabled`, the routing model may propose typed native memory tools, SSRF-filtered public web search/fetch tools, durable job tools, or manifest-grounded external app plans from registered tool schemas. Gigi validates every proposal through the runner policy, applies capability checks before tool execution, enforces channel visibility, and then either returns a dry-run plan or executes only allowed read-only/public-safe actions through the executor. Write-class tools are confirmation-required by default. Tool results can flow into an answerer that composes the final response, with short-term per-user/channel follow-up context for prompts such as "who said it?".

The core invariant is: LLM output is only a proposal. Gigi builds final action plans from registered native tool schemas, stored manifests, capability checks, confirmation policy, channel visibility rules, and audit rules. Deterministic tools such as `memory.count`, `memory.search`, `plugins.plan`, `permissions.check`, `web.search`, `web.fetch`, durable job queries, and usage summaries remain the source of truth. Natural language changes the path into those tools; it does not replace them. Web fetches are limited to public HTTP(S) hosts, blocked from loopback/private/link-local addresses and private redirects, capped before parsing, and limited to text-like content. Durable job writes stay confirmation-gated; the worker foundation has no product-specific job handlers by default.

The runner is bounded. V0 defaults keep runs short: a small step limit, a small tool-call limit, and a small LLM-call budget. Every step appends a redacted trace entry and, when the SQL store is wired, a durable run-step record with step/tool/LLM counters. Traces and audit metadata may include request IDs, run IDs, ordered step indexes, tool names, tool kind, status, capability, scope, result counts, source IDs, citation labels, restore handles, and context-pack status markers, but not raw prompts, raw snippets, provider payloads, or secrets. The durable schema includes run status, termination reason, budget settings, step observations, cancellation status, and pending confirmation records; completion preserves concurrent cancellation requests. Discord exposes `/agent` run management for canceling runs and resolving pending confirmations by the run owner or a server administrator, plus `/agent stats guild` for `agent.analytics` holders to view aggregate guild run health across fixed windows. Stats are aggregate-only: counts, latency summaries, status counts, termination reasons, and top tool names may be shown, but user IDs, per-user prompts, raw snippets, provider responses, tool arguments, planned commands, secrets, and individual run details stay out of the response. The same stats command can toggle the opt-in reply latency footer through `/agent stats guild reply-latency:on|off` for `agent.reply_latency.manage` holders; that footer exposes only response elapsed wall time and is separate from `/llm usage guild`, which remains the aggregate token/provider usage view.
...
- DM routing only has `ping` plus a response that rich chat needs a server LLM profile first. Guild mentions can dry-run enabled external app prefix triggers, dispatch consented public `send_message` triggers, propose semantic dry-run plans, or answer through a configured chat model.
...
- External app command dispatch is limited to public `send_message` prefix actions declared by approved enabled manifests with stored import-time public-dispatch consent. Restricted actions stay dry-run only.
...
- Guild memory is limited to opted-in current-channel count/search/recent behavior with cited evidence payloads and gateway delete-event tombstones; cross-channel semantic retrieval, backfill UX, and export workflows remain later behavior.
