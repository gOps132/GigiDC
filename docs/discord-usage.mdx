...
- message-triggered commands when an approved installed plugin declares a prefix trigger
- plugin manifests that declare triggers, surfaces, permissions, exact Discord identity, and attribution
...
Approved plugin manifest validation, storage, import, enable, and disable controls are started through `/plugins`. Plugin discovery is exact-match only by Discord application ID or bot user ID, or by an explicitly approved HTTPS manifest URL or uploaded JSON manifest. Gigi does not scrape plugin docs, guess plugin behavior from names, or execute plugin commands yet.
