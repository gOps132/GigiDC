...
Approved plugin manifest validation and storage are started, but there is no Discord `/plugins` command yet. Plugin discovery is exact-match only by Discord application ID or bot user ID, or by an explicitly approved HTTPS manifest URL. Gigi does not scrape plugin docs or guess plugin behavior from names.
