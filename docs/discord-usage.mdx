...
The current codebase has a disabled-by-default Discord gateway adapter, a minimal `/ping` slash-command handler, opt-in command sync, DM routing, guild-mention routing, admin-gated `/permissions` capability grants, `/plugins` catalog controls, deterministic external app dry-run matching, and limited public external app dispatch. It intentionally does not run LLM chat, memory retrieval, task actions, relay actions, or restricted external app dispatch yet.
...
- Guild mention text can match an enabled external app manifest prefix trigger. Dry-run manifests return the planned command. Public manifests with `dispatch: "send_message"` send the planned command as Gigi.
