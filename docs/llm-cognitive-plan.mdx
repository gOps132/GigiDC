...
- Initial `internal/contextbroker` context packer seam for bounded snippet packing, now extended into a typed pack builder with stable source IDs, citation labels, stale/invalidation markers, pinned snippets, omitted-unchanged records, restore handles, and next-state fingerprints.
...
- A general `AgentRunner` with `MaxSteps`, `MaxToolCalls`, LLM-call budget, and redacted trace. Started with a compatibility runner that preserves current mention and `/ask` behavior while moving orchestration out of one-shot planning handlers, and now persists run lifecycle and step observations when a run store is available. Discord responses now carry run IDs for confirmation-required runs, and `/agent` can cancel, inspect, confirm, or reject pending records.
...
- A context broker that decides whether a question needs no context, current-channel context, permitted guild memory, or enabled plugin catalog context. Started with bounded context pack building, restore-safe status metadata, and a first permitted recent-channel memory provider; richer retrieval-provider wiring remains incremental.
- A token packer that reserves budget for system instructions, tool results, citations, and output, then trims context deterministically. Started with deterministic character-budget snippet packing, pinned snippet priority, citation labels, and omitted-unchanged restore handles.
...
- Audits and durable steps for `agent.plan`, `agent.tool`, `agent.answer`, and `agent.confirmation` using request IDs, run IDs, ordered step indexes, status, scopes, tool names, tool kind, capability, token counts, termination reasons, and redacted metadata only.
