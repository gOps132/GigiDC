...
- A general `AgentRunner` with `MaxSteps`, `MaxToolCalls`, LLM-call budget, and redacted trace. This runner should replace one-shot planning internals while preserving current behavior.
- A split policy/executor/answerer core: policy owns routing mode, context scope, capability checks, and confirmation rules; executor owns tool lookup, dry-run behavior, execution, masked errors, and trace events; answerer owns final response synthesis.
...
- Audits for `agent.plan`, `agent.tool`, `agent.answer`, and `agent.confirmation` using request IDs, status, scopes, tool names, token counts, and redacted metadata only.
