...
- Initial `internal/agent` runtime contracts, planner handler, native tool registry, memory count/search tools, and chat fallback adapter.
...
- A native tool registry with at least `memory.count`, `memory.search`, `memory.recent`, `plugins.plan`, and `llm.chat`. Started with `memory.count`, `memory.search`, and chat fallback.
- A context broker that decides whether a question needs no context, current-channel context, permitted guild memory, or enabled plugin catalog context.
- A token packer that reserves budget for system instructions, tool results, citations, and output, then trims context deterministically.
