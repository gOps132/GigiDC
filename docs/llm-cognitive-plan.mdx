...
- Initial `internal/agent` runtime contracts, planner handler, native tool registry, memory count/search/recent tools, and chat fallback adapter.
...
- A typed planner contract for intents, context requests, tool calls, clarification questions, and confirmation requirements. Started for memory planning.
...
- A split policy/executor/answerer core. Started with policy-owned routing mode and capability checks, executor-owned tool lookup/execution/masked errors, deterministic write-tool confirmation guards, answerer-owned synthesis hooks, and audit trace metadata for `run_id`, `step_index`, tool kind, and capability.
- A native tool registry with at least `memory.count`, `memory.search`, `memory.recent`, `plugins.plan`, and `llm.chat`. Started with `memory.count`, `memory.search`, `memory.recent`, and chat fallback.
...
- Read-only tool execution for memory and usage-style queries after capability and channel visibility checks. Started with current-channel memory tools and capability checks.
