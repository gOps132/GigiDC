...
Current V0 provider foundation includes:
...
- Provider-backed text client implementations that consume resolved model credentials.
- Minimal cognitive fallback chat for DMs and guild mentions after deterministic plugin matching fails.
- Metadata-only conversation turn storage is in place; raw memory/retrieval policy remains deferred.
- Semantic plugin routing in dry-run or confirmation-required mode only.
- Policy fields for personal keys defaulted to `off`, or an explicit deferral before V1 personal BYOK.
