...
- A single `AgentHandler` fallback behind deterministic external app matching.
- A typed planner contract for intents, context requests, tool calls, clarification questions, and confirmation requirements.
- A native tool registry with at least `memory.count`, `memory.search`, `memory.recent`, `plugins.plan`, and `llm.chat`.
...
- Read-only tool execution for memory and usage-style queries after capability and channel visibility checks.
