...
- Initial `internal/contextbroker` context fetcher/packer seam for current-channel retained memory and bounded snippet packing.
...
- A context broker that decides whether a run needs no context or current-channel memory context, fetches only enabled-channel retained messages after capability and channel checks, and returns a bounded pack for answer synthesis. Cross-channel guild context and plugin catalog context remain future slices.
- A token packer that reserves budget for tool results, fetched context snippets, citations/source IDs, and output, then trims snippets deterministically. Current implementation uses character budgets until token accounting lands.
